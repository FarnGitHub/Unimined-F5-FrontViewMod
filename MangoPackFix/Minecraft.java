...
import farn.frontview.FrontViewHelper;
...
public abstract class Minecraft implements Runnable {
	...
	public void run() {
		...
		try {
			...
			this.onMinecraftCrash(new UnexpectedThrowable("Failed to start game", exception18));
			return;
		}
			...
			while(this.running) {
				try {
					...
					for(int i6 = 0; i6 < this.timer.elapsedTicks; ++i6) {
						...
						try {
							this.runTick();
						} catch (MinecraftException minecraftException17) {
							this.theWorld = null;
							this.changeWorld1((World)null);
				...
				} catch (MinecraftException minecraftException19) {
					this.theWorld = null;
					this.changeWorld1((World)null);
					this.displayGuiScreen(new GuiConflictWarning());
				} catch (OutOfMemoryError outOfMemoryError20) {
					this.func_28002_e();
					this.displayGuiScreen(new GuiErrorScreen());
					System.gc();
				} catch (RuntimeException runtimeException21) {
					runtimeException21.printStackTrace();
					this.changeWorld1((World)null);
					this.displayGuiScreen(new GuiException(runtimeException21));
				}
			}
		} catch (MinecraftError minecraftError22) {
		} catch (Throwable throwable23) {
			this.func_28002_e();
			throwable23.printStackTrace();
			this.onMinecraftCrash(new UnexpectedThrowable("Unexpected error", throwable23));
		} finally {
	...
	public void runTick() {
		...
		int i4;
		if(this.thePlayer != null) {
			IChunkProvider iChunkProvider2 = this.theWorld.getIChunkProvider();
			if(iChunkProvider2 instanceof ChunkProviderLoadOrGenerate) {
				ChunkProviderLoadOrGenerate chunkProviderLoadOrGenerate3 = (ChunkProviderLoadOrGenerate)iChunkProvider2;
				i1 = MathHelper.floor_float((float)((int)this.thePlayer.posX)) >> 4;
				i4 = MathHelper.floor_float((float)((int)this.thePlayer.posZ)) >> 4;
				chunkProviderLoadOrGenerate3.setCurrentChunkOver(i1, i4);
			}
		}
	...
	private void func_6255_d(String string1) {
		...
		for(int i9 = -s2; i9 <= s2; i9 += 16) {
			for(int i8 = -s2; i8 <= s2; i8 += 16) {
				this.loadingScreen.setLoadingProgress(i3++ * 100 / i4);
				this.theWorld.getBlockId(chunkCoordinates6.x + i9, 64, chunkCoordinates6.z + i8);
	...
	public static void startMainThread(String string0, String string1, String string2) {
		...
		if(string0 != null && string1 != null) {
			minecraftImpl6.session = new Session(string0, string1);
		} else {
			minecraftImpl6.session = new Session("Player" + System.currentTimeMillis() % 1000L, "");
		}

		if(string2 != null) {
			String[] string8 = string2.split(":");
	...
	/*static final class EnumOSMappingHelper {
